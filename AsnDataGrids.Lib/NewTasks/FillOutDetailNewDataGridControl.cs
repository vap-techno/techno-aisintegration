...
{
    public partial class FillOutDetailNewDataGridControl : UserControl
    {
        ...
        private readonly int _cmdType = 3; // Слив из АЦ
      ...
      WHERE [FillOutDetail].[Fs] = 1 
        ";

        private const string SqlSort = "\n ORDER BY [FillOutTask].[Tdt] DESC";

        private const string SqlDay = SqlAll + "AND [FillOutTask].[Tdt] > DATEADD(day,-1,GETDATE())" + SqlSort;
        private const string SqlWeek = SqlAll + "AND [FillOutTask].[Tdt] > DATEADD(WEEK,-1,GETDATE())" + SqlSort;
        private const string SqlMonth = SqlAll + "AND [FillOutTask].[Tdt] > DATEADD(month,-1,GETDATE())" + SqlSort;
        private const string SqlYear = SqlAll + "AND [FillOutTask].[Tdt] > DATEADD(year,-1,GETDATE())" + SqlSort;

        #endregion

        #region Constructors
        public FillOutDetailNewDataGridControl()
        {
            InitializeComponent();
