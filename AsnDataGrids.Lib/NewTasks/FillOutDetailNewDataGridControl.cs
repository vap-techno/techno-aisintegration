...
namespace AsnDataGrids.Lib.NewTasks
{
    public partial class FillOutDetailNewDataGridControl : UserControl
    {
      ...
      ON FillOutDetail.FillOutTaskId = FillOutTask.FillOutTaskId
        ...
        private const string SqlDay = SqlAll + "\n WHERE [FillOutDetail].[Fs] = 1 AND [FillOutTask].[Tdt] > DATEADD(day,-1,GETDATE())" + SqlSort;
        private const string SqlWeek = SqlAll + "\n WHERE [FillOutDetail].[Fs] = 1 AND [FillOutTask].[Tdt] > DATEADD(WEEK,-1,GETDATE())" + SqlSort;
        private const string SqlMonth = SqlAll + "\n WHERE [FillOutDetail].[Fs] = 1 AND [FillOutTask].[Tdt] > DATEADD(month,-1,GETDATE())" + SqlSort;
        private const string SqlYear = SqlAll + "\n WHERE [FillOutDetail].[Fs] = 1 AND [FillOutTask].[Tdt] > DATEADD(year,-1,GETDATE())" + SqlSort;
        ...
        private string GetQuerySqlStringCustom(DateTime beginDateTime, DateTime endDateTime)
        {
            ...
            return SqlAll +
                   " \n WHERE [FillOutDetail].[Fs] = 1 AND [FillOutTask].[Tdt] BETWEEN CAST('" +
