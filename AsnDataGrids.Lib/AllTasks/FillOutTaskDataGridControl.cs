...
{
    public partial class FillOutTaskDataGridControl : UserControl
    {
        #region Fields
        ...
        {
            @"C:\Project\Config\", @"C:\Program Files (x86)\Siemens\Automation\WinCC RT Advanced",
      ...
      , [Mm].[Name] as 'Способ измерения'
       FROM [FillOutTask]
       INNER JOIN [Mm]
       ON [Mm].[MmId] = [FillOutTask].[Mm]
        ";

        private const string SqlSort = "\n ORDER BY Tdt DESC";

        private const string SqlDay = SqlAll + "\n WHERE Tdt > DATEADD(day,-1,GETDATE())" + SqlSort;
        private const string SqlWeek = SqlAll + "\n WHERE Tdt > DATEADD(WEEK,-1,GETDATE())" + SqlSort;
        private const string SqlMonth = SqlAll + "\n WHERE Tdt > DATEADD(month,-1,GETDATE())" + SqlSort;
        private const string SqlYear = SqlAll + "\n WHERE Tdt > DATEADD(year,-1,GETDATE())" + SqlSort;

        #endregion

        public FillOutTaskDataGridControl()
        {
            InitializeComponent();
        ...
        private void ExportToExcel()
        {
            ...
            try
            {
                ...
                {
                    Filter = "Excel files (*.xlsx)|*.xlsx|All files (*.*)|*.*", FilterIndex = 2
                };
